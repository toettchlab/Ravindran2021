function [t, y] = Run_Circuit_6(Times,params)
...
dx1dt = @(x1,u) k1*(A.^n1/(A.^n1+u.^n1) - x1);
dx2dt = @(x1,x2,u) k2*((1 - (1-(B.^n2./(x1.^n2+B^n2))).* (1-(A.^n3./(A.^n3+u.^n3)))) - x2);
%dx2dt = @(x1,x2,u) k2*(B.^n2./(x1.^n2+B^n2).*A.^n3./(A.^n3+u.^n3) - x2);
